...
  try {
    // *** CRITICAL CORRECTION: Use the Netlify Function path ***
    const response = await fetch("/.netlify/functions/server/api/submit-quiz", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload)
    });
...
    const data = await response.json();
    ...
    if (!response.ok) {
        console.error("Server submission failed with status:", response.status, data);
...
async function resetQuiz() {
  try {
        // *** CRITICAL CORRECTION: Use the Netlify Function path and send data ***
        const visitorID = document.getElementById("user-id-input").value.trim();
        await fetch("/.netlify/functions/server/api/save-click", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ action: 'retake', visitorID: visitorID }) 
        });
        // We no longer rely on the returned data.totalClicks being accurate here
    } catch (err) {
        console.error("Failed to register retake click:", err);
    }
