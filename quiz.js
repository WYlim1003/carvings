...
import { translations, QUIZ_QUESTIONS, CORRECT_ANSWERS, applyLanguage } from './script.js';
...
async function submitQuiz() {
    ...
    try {
      ...
      // Map payload to match database column names
      const dbPayload = {
        visitor_id: payload.visitorID,
        q1: payload.question1,
        q2: payload.question2,
        q3: payload.question3,
        q4: payload.question4,
        score: payload.score,
      ...
      // Show error but don't block user from seeing results
      setTimeout(() => {
        alert("Your results are displayed below. Note: Could not save to database.\n\nError: " + err.message);
      }, 500);
    } finally {
      if (submitBtn) {
        submitBtn.disabled = false;
...
function showResults(score, answers) {
  try {
    ...
    console.log("Results container found, classes before:", resultsContainer.className);
    ...
    // Show results
    resultsContainer.classList.remove("hidden");
    console.log("Results container classes after:", resultsContainer.className);
    ...
    if (resultsMessage) {
      let message = "";
      if (score === 4) {
        message = dict.excellentScore || "Excellent!";
      } else if (score >= 2) {
        message = dict.goodScore || "Good job!";
      } else {
        message = dict.lowScore || "Keep learning!";
      }
      resultsMessage.textContent = message;
    ...
    if (answersList) {
      ...
      answersList.innerHTML = questions.map((q, i) => {
        ...
        return `
          <div class="answer-item" style="margin:10px 0; padding:10px; border-left:4px solid ${isCorrect ? '#4CAF50' : '#f44336'}; background:#f9f9f9;">
            <p><strong>Question ${i+1}:</strong> ${isCorrect ? '✓ Correct' : '✗ Incorrect'}</p>
            <p>Your answer: ${answers[q].toUpperCase()} | Correct: ${CORRECT_ANSWERS[q].toUpperCase()}</p>
          </div>
        `;
      }).join('');
      console.log("Answers list updated");
    }
    ...
    console.log("=== SHOW RESULTS COMPLETE ===");
  } catch (err) {
    console.error("ERROR in showResults:", err);
    alert("Error showing results: " + err.message);
  }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("Checking for quiz elements...");
